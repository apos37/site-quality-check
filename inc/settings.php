...
class Settings {
     ...
     * Settings group name.
     */
    public const OPTION_GROUP = 'sqcheck_settings';
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_sqcheck_save_settings', [ $this, 'ajax_save_settings' ] );
        add_action( 'sqcheck_subheader_left', [ $this, 'render_subheader_buttons' ] );
    } // End __construct()
    ...
    public function register_settings() : void {
        register_setting( self::OPTION_GROUP, 'sqcheck_menu_title', [
            'type'              => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'default'           => __( 'Quality Check', 'site-quality-check' ),
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_page_title', [
            'type'              => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'default'           => __( 'Site Quality Check', 'site-quality-check' ),
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_menu_icon', [
            'type'              => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'default'           => 'dashicons-yes-alt',
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_menu_position', [
            'type'              => 'integer',
            'sanitize_callback' => 'absint',
            'default'           => 2,
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_logo', [
            'type'              => 'string',
            'sanitize_callback' => 'esc_url_raw',
            'default'           => '',
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_stale_thresholds', [
            'type'              => 'array',
            'sanitize_callback' => [ __CLASS__, 'sanitize_thresholds' ],
            'default'           => StaleContent::DEFAULT_THRESHOLDS,
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_stale_post_types', [
            'type'              => 'array',
            'sanitize_callback' => [ __CLASS__, 'sanitize_post_types' ],
            'default'           => [ 'post', 'page' ],
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_contact_page_id', [
            'type'              => 'integer',
            'sanitize_callback' => 'absint',
            'default'           => 0,
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_contact_form_id', [
            'type'              => 'integer',
            'sanitize_callback' => 'absint',
            'default'           => 0,
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_enabled_quick_actions', [
            'type'              => 'array',
            'sanitize_callback' => [ __CLASS__, 'sanitize_enabled_actions' ],
            'default'           => array_keys( QuickActions::get_available_actions() ),
        ] );

        register_setting( self::OPTION_GROUP, 'sqcheck_clear_data_on_uninstall', [
            'type'              => 'boolean',
    ...
    public function render_subheader_buttons( string $active_page ) : void {
        ...
        ?>
        <button type="submit" form="sqcheck-settings-form" class="sqcheck-button"><?php esc_html_e( 'Save', 'site-quality-check' ); ?></button>
        <span id="sqcheck-save-reminder"><?php esc_html_e( 'Remember to click "Save" after making changes to your settings.', 'site-quality-check' ); ?></span>
        <?php
    ...
    public static function render_page() : void {
        ...
        wp_localize_script( 'sqcheck-settings', 'sqcheckSettings', [
            'nonce'      => wp_create_nonce( 'sqcheck_settings_nonce' ),
        ...
        $contact_form_id = (int) get_option( 'sqcheck_contact_form_id', 0 );
        $enabled_actions = get_option( 'sqcheck_enabled_quick_actions', array_keys( QuickActions::get_available_actions() ) );
        ...
        $menu_icon = get_option( 'sqcheck_menu_icon', 'dashicons-yes-alt' );
        $logo = get_option( 'sqcheck_logo', '' );
        $allowed_roles = get_option( 'sqcheck_allowed_roles', [] );
        ?>
        <div class="wrap sqcheck-content-wrap sqcheck-settings">
            <?php Advanced::render_notices(); ?>
            <div class="sqcheck-settings-grid">

                <div class="sqcheck-box">
                    <div class="sqcheck-box-header"><h2><?php esc_html_e( 'Interface', 'site-quality-check' ); ?></h2></div>
                    <div class="sqcheck-box-body">
                        <div class="sqcheck-field">
                            <label><?php esc_html_e( 'Menu Title', 'site-quality-check' ); ?></label>
                            <input type="text" id="sqcheck_menu_title" name="sqcheck_menu_title" value="<?php echo esc_attr( $menu_title ); ?>">
                        </div>
                        <div class="sqcheck-field">
                            <label>
                            ...
                            <select id="sqcheck_menu_icon" name="sqcheck_menu_icon">
                        ...
                        <div class="sqcheck-field">
                            ...
                            <input type="text" id="sqcheck_page_title" name="sqcheck_page_title" value="<?php echo esc_attr( $page_title ); ?>">
                        </div>
                        <div class="sqcheck-field">
                            <label>
                            ...
                            <input type="text" id="sqcheck_logo" name="sqcheck_logo" value="<?php echo esc_attr( $logo ); ?>" placeholder="https://example.com/logo.png">
                        </div>
                    </div>
                </div>

                <div class="sqcheck-box">
                    <div class="sqcheck-box-header"><h2><?php esc_html_e( 'Stale Content Thresholds', 'site-quality-check' ); ?></h2></div>
                    <div class="sqcheck-box-body">
                        <?php foreach ( [ 'warning' => __( 'Warning (days)', 'site-quality-check' ), 'danger' => __( 'Danger (days)', 'site-quality-check' ), 'critical' => __( 'Critical (days)', 'site-quality-check' ) ] as $key => $label ) : ?>
                            <div class="sqcheck-field">
                                ...
                                <input type="number" min="1" name="sqcheck_stale_thresholds[<?php echo esc_attr( $key ); ?>]" value="<?php echo esc_attr( $thresholds[ $key ] ); ?>">
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
                ...
                <div class="sqcheck-box">
                    <div class="sqcheck-box-header"><h2><?php esc_html_e( 'Post Types to Check', 'site-quality-check' ); ?></h2></div>
                    <div class="sqcheck-box-body">
                        <div class="sqcheck-checkboxes">
                            <?php foreach ( $public_post_types as $post_type ) : ?>
                                <label>
                                    <input type="checkbox" name="sqcheck_stale_post_types[]" value="<?php echo esc_attr( $post_type->name ); ?>" <?php checked( in_array( $post_type->name, $included_post_types, true ), true ); ?>>
                ...
                <div class="sqcheck-box">
                    <div class="sqcheck-box-header"><h2><?php esc_html_e( 'Contact Page & Form', 'site-quality-check' ); ?></h2></div>
                    <div class="sqcheck-box-body">
                        <div class="sqcheck-field">
                            <label>
                ...
                <div class="sqcheck-box">
                    <div class="sqcheck-box-header"><h2><?php esc_html_e( 'Quick Actions', 'site-quality-check' ); ?></h2></div>
                    <div class="sqcheck-box-body">
                        <div class="sqcheck-checkboxes">
                            <?php foreach ( QuickActions::get_available_actions() as $slug => $action ) : ?>
                                ...
                                <label style="display: block; font-weight: 400;">
                                    <input type="checkbox" name="sqcheck_enabled_quick_actions[]" value="<?php echo esc_attr( $slug ); ?>" <?php checked( in_array( $slug, $enabled_actions, true ), true ); ?>>
                ...
                <div class="sqcheck-box">
                    <div class="sqcheck-box-header"><h2><?php esc_html_e( 'Access', 'site-quality-check' ); ?></h2></div>
                    <div class="sqcheck-box-body">
                        <div class="sqcheck-field">
                            <label>
                            ...
                            <div class="sqcheck-checkboxes">
                                <?php foreach ( Access::get_assignable_roles() as $slug => $label ) : ?>
                                    <label>
                                        <input type="checkbox" name="sqcheck_allowed_roles[]" value="<?php echo esc_attr( $slug ); ?>" <?php checked( in_array( $slug, $allowed_roles, true ), true ); ?>>
                ...
                <div class="sqcheck-box">
                    <div class="sqcheck-box-header"><h2><?php esc_html_e( 'Advanced', 'site-quality-check' ); ?></h2></div>
                    <div class="sqcheck-box-body">

                        <div class="sqcheck-field">
                            <label style="font-weight: 400; display: flex; align-items: center; gap: 6px;">
                                <input type="checkbox" name="sqcheck_clear_data_on_uninstall" value="1" <?php checked( get_option( 'sqcheck_clear_data_on_uninstall' ), true ); ?>>
                        ...
                        <div class="sqcheck-field" id="sqcheck_field_upload_download_settings">
                            <label>
                            ...
                            <div style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
                                <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
                                    <?php wp_nonce_field( 'sqcheck_advanced_action' ); ?>
                                    <input type="hidden" name="action" value="sqcheck_export">
                                    <button type="submit" class="sqcheck-button"><?php esc_html_e( 'Download Settings', 'site-quality-check' ); ?></button>
                                </form>
                                <label for="sqcheck-upload-settings" class="sqcheck-button" style="cursor: pointer;"><?php esc_html_e( 'Upload Settings', 'site-quality-check' ); ?></label>
                                <input type="file" id="sqcheck-upload-settings" accept="application/json" style="display: none;">
                                <span id="sqcheck-upload-filename"></span>
                            </div>
                        </div>

                        <div class="sqcheck-field" id="sqcheck_field_reset_settings">
                            <label>
                            ...
                            <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" onsubmit="return confirm('<?php echo esc_js( __( 'Reset all settings to defaults?', 'site-quality-check' ) ); ?>');">
                                <?php wp_nonce_field( 'sqcheck_advanced_action' ); ?>
                                <input type="hidden" name="action" value="sqcheck_reset_settings">
                                <button type="submit" class="sqcheck-button"><?php esc_html_e( 'Reset Settings', 'site-quality-check' ); ?></button>
                            </form>
                        </div>
    ...
    public function ajax_save_settings() : void {
        check_ajax_referer( 'sqcheck_settings_nonce', 'nonce' );
        ...
        $simple_fields = [
            'sqcheck_menu_title'      => 'sanitize_text_field',
            'sqcheck_page_title'      => 'sanitize_text_field',
            'sqcheck_menu_icon'       => 'sanitize_text_field',
            'sqcheck_logo'            => 'esc_url_raw',
            'sqcheck_contact_page_id' => 'absint',
            'sqcheck_contact_form_id' => 'absint',
        ];
        ...
        update_option( 'sqcheck_clear_data_on_uninstall', isset( $settings[ 'sqcheck_clear_data_on_uninstall' ] ) && $settings[ 'sqcheck_clear_data_on_uninstall' ] ? 1 : 0 );

        if ( isset( $settings[ 'sqcheck_stale_thresholds' ] ) && is_array( $settings[ 'sqcheck_stale_thresholds' ] ) ) {
            update_option( 'sqcheck_stale_thresholds', self::sanitize_thresholds( $settings[ 'sqcheck_stale_thresholds' ] ) );
        }

        if ( isset( $settings[ 'sqcheck_stale_post_types' ] ) ) {
            update_option( 'sqcheck_stale_post_types', self::sanitize_post_types( (array) $settings[ 'sqcheck_stale_post_types' ] ) );
        }

        if ( isset( $settings[ 'sqcheck_enabled_quick_actions' ] ) ) {
            update_option( 'sqcheck_enabled_quick_actions', self::sanitize_enabled_actions( (array) $settings[ 'sqcheck_enabled_quick_actions' ] ) );
        }

        if ( isset( $settings[ 'sqcheck_allowed_roles' ] ) ) {
            $allowed_roles = array_map( 'sanitize_key', (array) $settings[ 'sqcheck_allowed_roles' ] );
            update_option( 'sqcheck_allowed_roles', $allowed_roles );
        }
