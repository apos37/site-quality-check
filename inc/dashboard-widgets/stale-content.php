...
function render_stale_content_widget() : void {
    ...
    echo '<ul class="sqc-stale-counts">';
    echo '<li><span class="sqc-badge sqc-badge-warning">' . esc_html( $counts[ 'warning' ] ) . '</span> ' . esc_html__( 'over 6 months', 'site-quality-check' ) . '</li>';
    echo '<li><span class="sqc-badge sqc-badge-danger">' . esc_html( $counts[ 'danger' ] ) . '</span> ' . esc_html__( 'over 1 year', 'site-quality-check' ) . '</li>';
    echo '<li><span class="sqc-badge sqc-badge-critical">' . esc_html( $counts[ 'critical' ] ) . '</span> ' . esc_html__( 'over 2 years', 'site-quality-check' ) . '</li>';
    echo '</ul>';

    $url = admin_url( 'admin.php?page=site-quality-check-stale-content' );
    echo '<p><a href="' . esc_url( $url ) . '">' . esc_html__( 'View stale content', 'site-quality-check' ) . '</a></p>';
